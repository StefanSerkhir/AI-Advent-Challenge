...
internal const val MCP_DEMO_SERVER_NAME = "llm-workbench-mcp-orchestrator"
internal const val MCP_DEMO_VERSION = "2.0.0"
...
internal val MCP_TOOL_NAMES_BY_SERVER = mapOf(
    OPERATIONS_SERVER_ID to setOf(
        "ping", "echo", TRACKER_GET_ISSUE_TOOL,
        SCHEDULER_CREATE_TOOL, SCHEDULER_LIST_TOOL, SCHEDULER_CANCEL_TOOL, SCHEDULER_SUMMARY_TOOL,
    ),
    KNOWLEDGE_SERVER_ID to setOf(SEARCH_TOOL, SUMMARIZE_TOOL),
    WORKSPACE_SERVER_ID to setOf(SAVE_TO_FILE_TOOL),
)
...
public fun main(): Unit = runBlocking {
    val serverId = System.getenv("LLM_MCP_SERVER_ID")
        ?.takeIf(String::isNotBlank)
        ?: error("LLM_MCP_SERVER_ID is required")
    require(serverId in MCP_TOOL_NAMES_BY_SERVER) { "Unknown MCP server registration." }
    val outputDirectory = System.getenv("LLM_MCP_OUTPUT_DIR")
        ?.takeIf(String::isNotBlank)
        ?.let(Path::of)
        ?: Path.of(DEFAULT_MCP_OUTPUT_DIRECTORY)
    val scheduler = if (serverId == OPERATIONS_SERVER_ID) {
    ...
    val server = createDemoServer(serverId, scheduler, outputDirectory)
...
internal fun createDemoServer(
    ...
    serverInfo = Implementation(
        name = "$MCP_DEMO_SERVER_NAME-$serverId",
    ...
    if (serverId == KNOWLEDGE_SERVER_ID) {
        addTool(
            name = SEARCH_TOOL,
    ...
    if (serverId == OPERATIONS_SERVER_ID) {
        val operationsScheduler = checkNotNull(scheduler) { "Operations server requires SchedulerService." }
        addTool(
            ...
            safeToolCall {
                ...
                val schedule = operationsScheduler.create(SchedulerCreateCommand(
                ...
                val result = operationsScheduler.summaries(schedule.id).toJson()
        ...
        addTool(
            ...
            safeToolCall {
                ...
                structuredResult(operationsScheduler.summaries().toJson())
            }
        }
        ...
        addTool(
            ...
            safeToolCall {
                ...
                val cancelled = operationsScheduler.cancel(args.requiredString("scheduleId"))
                structuredResult(operationsScheduler.summaries(cancelled.id).toJson())
            }
        }
        ...
        addTool(
            ...
            safeToolCall {
                val args = request.arguments ?: buildJsonObject {}
                requireOnlyKeys(args, setOf("scheduleId"))
                structuredResult(operationsScheduler.summaries(args.optionalString("scheduleId")).toJson())
            }
        }
    }
}
