...
- Backend: Kotlin/JVM 21, Ktor, coroutines, kotlinx.serialization и Kotlin MCP SDK; исходники находятся в `src/main/kotlin`.
...
- Три слоя и профиль `MEMORY_LAYERS` принадлежат `AssistantAgent`/`AssistantMemoryManager` и `AssistantMemoryStore`; инварианты и состояние задачи обслуживаются отдельными `AssistantInvariantManager`/`AssistantInvariantStore` и `TaskStateManager`/`TaskStateStore`. Не используйте `slidingMessages`, `stickyFacts`, `branching` или `.llm-history.json` как скрытое хранилище этих данных.
...
- В `MEMORY_LAYERS` порядок контекста: system instructions с непустыми инвариантами, профилем и активной незавершённой задачей → `LONG_TERM` → `WORKING` → `SHORT_TERM` → текущий user prompt. Приоритет: безопасность/system → инварианты → текущий prompt → task state → профиль → остальные слои. Пустые или выключенные слои не отправляются, текущий prompt не дублируется.
...
Не передавайте доменные Kotlin-объекты напрямую по wire: API использует явные DTO. Для настроек и доменных мутаций, зависящих от версии состояния, сохраняйте optimistic concurrency через `expectedSettingsVersion`; запуск должен оставаться идемпотентным по `requestId`. SSE-снимок сервера — источник истины для UI. Изменения MCP-каталога, tool-calling, scheduler или diagnostics сверяйте также с `mcp`, LLM transport, `StateDto`, fixture и MCP/API/browser-тестами.
