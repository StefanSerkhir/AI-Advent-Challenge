...
обмен сначала атомарно сохраняется в `.llm-history.json`, затем публикуется в
памяти и API. При ошибке или отмене последний накопленный черновик остаётся в
...
возвращает `500 persistence` и сохраняет прежнюю историю.
